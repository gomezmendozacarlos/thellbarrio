// Concerts functionality for Thell Barrio Landing Page
// Sistema dinámico de fechas de conciertos
// Version 2.0 - New mobile-first layout
 ...
 * En un proyecto real, esto vendría de una API o CMS
 */
const concertsData = [
        {
        id: 1,
...
/**
 * Initialize concerts functionality
 */
...
function renderConcerts() {
    ...
    const futureConcerts = concertsData.filter(concert => {
        // Concert is future if:
        // 1. Year is greater than current year
        // 2. Same year but month is greater than current month
        // 3. Same year and month but day is greater or equal to current day
    ...
    // Hide no concerts message
    noConcertsMessage?.classList.add('hidden');
    concertsContainer.classList.remove('hidden');
    
    // Sort concerts by date (earliest first) - using year, month, day
...
function createConcertElement(concert) {
    ...
    // Format date - Mobile first (simplified system)
    const monthNames = ['', 'ene', 'feb', 'mar', 'abr', 'may', 'jun', 'jul', 'ago', 'sep', 'oct', 'nov', 'dic'];
    const dayNames = ['dom', 'lun', 'mar', 'mié', 'jue', 'vie', 'sáb'];
    
    // Create a date object to get day of week using the concert's year
...
/**
 * Create ticket button based on availability
 */
...
/**
 * Track ticket clicks for analytics
 */
function trackTicketClick(concertId) {
    // En un proyecto real, aquí se enviarían eventos a Google Analytics
    console.log(`Ticket click tracked for concert ID: ${concertId}`);
    
    // Example: gtag('event', 'click', { 'event_category': 'tickets', 'event_label': concertId });
}

/**
 * Add new concert (for admin functionality)
 */
...
// Export functions for potential admin interface
